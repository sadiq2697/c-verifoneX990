...
namespace PayECR
{
    public class ECR
    {
        ...
        public void SendReceive(string writedata, ref string readdata, ref int status_code, ref string status_remark, int receive_buffer_size)
        {
            try
            {
                using (_serialPort = new SerialPort(gs_com, gi_baudRate, g_parity, gs_databits, g_stopbits))
                {
                    ...
                    ls_send = "\x02";
                    status_code = 0;
                    if (writedata.Contains("C101") || writedata.Contains("C230"))
                    {
                        CarPark = true;
                    }
        ...
        private bool ReadFromCOM(ref int status_code, ref string status_remark, ref string readdata)
        {
            ...
            int t = 0;// to start the loop//  added to test kiosk purpose

            while (t == 0)// for added to test kiosk purpose
            {
                ...
                if (CarPark)
                {
                    if (_serialPort.BytesToRead == 27)
                    {
                        li_response = RcvData(recebuff, 0, 27);
                        Log("ReadFromCOM:-Read " + asciiOctets2String((System.Text.Encoding.UTF8.GetBytes(System.Text.ASCIIEncoding.ASCII.GetString(recebuff).Replace("\0", String.Empty).Trim()))));
                    }
                    else
                    {
                        CarPark = false;
                ...
                else if (_serialPort.BytesToRead == 4)
                {
                    li_response = RcvData(recebuff, 0, 4);
                    Log("ReadFromCOM:-Read " + asciiOctets2String((System.Text.Encoding.UTF8.GetBytes(System.Text.ASCIIEncoding.ASCII.GetString(recebuff).Replace("\0", String.Empty).Trim()))));
        ...
        private bool computeLRCforByte(byte[] str)
        {
            ...
            //get etx position
            for (int i = str.Length - 1; i > li_stx; i--)
            {
                if (str[i] == lc_etx)
                {
                    li_etx = i;
            ...
            for (int i = li_stx + 1; i < li_etx + 1; i++)
            {
                LRC = LRC ^ str[i];
